export default {
    common: {
        dashboard: '仪表板',
        endpoints: '端点',
        statistics: '统计',
        testing: '测试',
        toggleTheme: '切换主题',
        toggleLanguage: '切换语言',
        add: '添加',
        edit: '编辑',
        delete: '删除',
        save: '保存',
        update: '更新',
        create: '创建',
        cancel: '取消',
        confirm: '确认',
        close: '关闭',
        loading: '加载中...',
        noData: '暂无数据',
        name: '名称',
        status: '状态',
        actions: '操作',
        enabled: '启用',
        disabled: '禁用',
        active: '活动',
        inactive: '非活动',
        success: '成功',
        failed: '失败',
        error: '错误',
        warning: '警告',
        info: '信息',
        total: '总计',
        refresh: '刷新',
        copy: '复制',
        clone: '克隆',
        test: '测试',
        switch: '切换',
        import: '导入',
        export: '导出',
        required: '必填',
        optional: '可选',
        skipToContent: '跳到主要内容',
        switchToLightTheme: '切换到浅色主题',
        switchToDarkTheme: '切换到深色主题',
        pageTitle: 'ccNexus - 管理面板',
        unknown: '未知'
    },
    dashboard: {
        title: '仪表板',
        subtitle: 'AI 代理管理',
        totalRequests: '总请求数',
        successRate: '成功率',
        inputTokens: '输入令牌',
        outputTokens: '输出令牌',
        activeEndpoints: '活动端点',
        recentActivity: '最近活动',
        requests: '请求数',
        noEndpoints: '未配置端点',
        noEnabledEndpoints: '无启用端点',
        failedToLoad: '加载仪表板数据失败',
        chartUnavailable: '图表暂不可用，统计表格仍可正常查看。',
        noActivityData: '暂无活动数据。'
    },
    endpoints: {
        title: '端点管理',
        addEndpoint: '添加端点',
        name: '名称',
        apiUrl: 'API 地址',
        apiKey: 'API 密钥',
        transformer: '转换器',
        model: '模型',
        tokenPool: '令牌池',
        status: '状态',
        actions: '操作',
        current: '当前',
        notTested: '未测试',
        testPassed: '测试通过',
        testFailed: '测试失败',
        switchToEndpoint: '切换到此端点',
        copyUrl: '复制 URL',
        tokenPoolManagement: '令牌池管理',
        deleteEndpoint: '删除端点',
        cloneEndpoint: '克隆端点',
        copySuffix: '（副本）',
        noEndpoints: '暂无端点',
        noEndpointsMessage: '添加您的第一个端点以开始使用',
        editEndpoint: '编辑端点',
        remark: '备注',
        fetchModels: '获取模型',
        fetchModelsHint: '点击"获取模型"从 API 加载可用模型',
        codexModelsUnavailable: 'Codex 令牌池端点不支持获取模型列表。',
        apiKeyModeHint: '请求使用已保存的 API 密钥进行认证。',
        tokenPoolModeHint: '保存端点后，可从令牌池管理凭证。',
        codexTokenPoolModeHint: 'Codex 地址和 Responses 转换器由系统自动管理。',
        modelPlaceholder: 'gpt-4、gemini-pro 等',
        apiKeyPlaceholder: 'sk-...',
        apiUrlPlaceholder: 'https://api.example.com',
        keepExistingKey: '留空可保留已保存的密钥',
        selectModel: '选择模型',
        noModelsFound: '未找到模型',
        testEndpoint: '测试端点',
        testModel: '选择测试模型',
        testModelHelp: '该模型仅用于本次测试，不会保存或影响正式请求。',
        testModelRequired: '请输入测试模型',
        testing: '测试中...',
        testSuccessful: '测试成功！延迟：',
        testResult: '测试结果',
        latency: '延迟',
        response: '响应',
        noResponse: '无响应',
        confirmDelete: '确定要删除端点"{name}"吗？',
        endpointCreated: '端点创建成功',
        endpointUpdated: '端点更新成功',
        endpointDeleted: '端点删除成功',
        endpointCloned: '端点克隆成功',
        endpointSwitched: '已切换到端点：',
        endpointEnabled: '端点已启用',
        endpointDisabled: '端点已禁用',
        endpointsReordered: '端点重新排序成功',
        failedToLoad: '加载失败',
        failedToSave: '保存失败',
        failedToDelete: '删除失败',
        failedToToggle: '切换失败',
        failedToSwitch: '切换失败',
        failedToReorder: '重新排序失败',
        failedToTest: '测试失败',
        failedToFetchModels: '获取模型失败',
        failedToClone: '克隆失败',
        failedToCopy: '复制到剪贴板失败',
        urlCopied: 'URL 已复制',
        enterApiUrlAndKey: '请输入 API 地址和 API 密钥，或先保存端点',
        modelSelected: '已选择模型：',
        tokenPoolTitle: '令牌池：',
        total: '总计',
        active: '活动',
        expiring: '即将过期',
        needRefresh: '需要刷新',
        expired: '已过期',
        invalid: '无效',
        cooldown: '冷却中',
        activate: '激活',
        batchImportJson: '批量导入 JSON',
        jsonPasteHint: '粘贴一项 / 数组 / {"items":[...]}',
        overwriteExisting: '覆盖现有的 account_id/email',
        import: '导入',
        id: 'ID',
        account: '账户',
        email: '邮箱',
        expiresAt: '过期时间',
        lastError: '最后错误',
        noCredentials: '未导入凭证',
        importDone: '导入完成：新增 {created}，更新 {updated}，跳过 {skipped}，失败 {failed}',
        credentialEnabled: '凭证已启用',
        credentialDisabled: '凭证已禁用',
        credentialActivated: '凭证已激活',
        tokenUpdated: '凭证令牌已更新',
        noCredentialChanges: '凭证没有需要保存的更改。',
        credentialDeleted: '凭证已删除',
        failedToLoadTokenPool: '加载令牌池失败',
        failedToImport: '导入失败',
        failedToUpdateCredential: '更新凭证失败',
        failedToActivateCredential: '激活凭证失败',
        failedToDeleteCredential: '删除凭证失败',
        enterAccessToken: '新访问令牌',
        enterExpiresAt: '过期时间（RFC3339，可选）',
        confirmDeleteCredential: '确定要删除凭证 #{id} 吗？',
        pleasePasteJson: '请先粘贴凭证 JSON',
        invalidJson: '无效的 JSON',
        failedToUpdateToken: '更新令牌失败',
        reorder: '排序',
        authMode: '认证模式',
        apiKeyConfigured: '已配置 API 密钥',
        apiKeyMissing: '未配置 API 密钥',
        clearApiKey: '清除已保存的 API 密钥',
        clearApiKeyTitle: '确认清除 API 密钥',
        confirmClearApiKey: '保存后将删除已存储的 API 密钥，端点可能立即无法使用。确定继续吗？',
        apiKeyRequired: '新建 API 密钥端点时必须填写 API 密钥。',
        fetchingModels: '获取中...',
        tokenPoolUnavailable: '令牌池仅适用于令牌池认证模式。',
        updateCredential: '更新凭证令牌',
        deleteCredential: '删除凭证',
        tokens: '令牌',
        accessToken: '访问令牌',
        refreshToken: '刷新令牌',
        idToken: 'ID 令牌',
        keepExistingToken: '留空可保留已保存的令牌',
        tokenConfigured: '已配置令牌',
        tokenMissing: '未配置令牌',
        clearRefreshToken: '清除已保存的刷新令牌',
        clearIdToken: '清除已保存的 ID 令牌',
        credentialNotFound: '凭证已不存在，请刷新后重试。',
        collapse: '收起',
        expand: '展开',
        viewMode: '端点视图',
        detailView: '详细视图',
        compactView: '紧凑视图',
        filters: '端点筛选',
        filterTransformer: '按转换器筛选',
        filterAvailability: '按可用性筛选',
        filterEnabled: '按启用状态筛选',
        allTransformers: '全部类型',
        allAvailability: '全部可用性',
        allEnabledStates: '全部状态',
        available: '可用',
        unavailable: '不可用',
        unknownAvailability: '未测试',
        filterActive: '筛选已启用，排序暂不可用',
        clearFilters: '清除筛选',
        noFilterResults: '没有符合筛选条件的端点'
    },
    settings: {
        title: '设置',
        port: '端口',
        portLocked: '端口已由启动参数锁定。',
        portRestart: '修改端口后需要重启服务。',
        logLevel: '日志级别',
        logLevel0: '调试',
        logLevel1: '信息',
        logLevel2: '警告',
        logLevel3: '错误',
        basicAuth: '启用 Basic Auth',
        username: '用户名',
        password: '密码',
        passwordKeep: '留空可保留当前密码。',
        passwordRequired: '启用 Basic Auth 时必须设置密码。',
        authRequired: '启用 Basic Auth 时必须填写用户名和密码。',
        saved: '设置已保存',
        failedToLoad: '加载设置失败',
        failedToSave: '保存设置失败'
    },
    errors: {
        invalidJSONResponse: '服务器返回了无效的 JSON 响应',
        requestFailed: '请求失败（{status}）',
        networkError: '网络连接失败，请检查服务是否可用'
    },
    stats: {
        title: '统计',
        daily: '每日',
        weekly: '每周',
        monthly: '每月',
        totalRequests: '总请求数',
        successful: '成功',
        errors: '错误',
        totalTokens: '总令牌数',
        endpoints: '端点',
        activeTotal: '活动 / 总数',
        input: '输入',
        output: '输出',
        endpointBreakdown: '端点明细',
        endpoint: '端点',
        requests: '请求数',
        inputTokens: '输入令牌',
        outputTokens: '输出令牌',
        noDataAvailable: '暂无数据',
        failedToLoad: '加载统计数据失败'
    },
    testing: {
        title: '端点测试',
        endpoint: '端点',
        selectEndpoint: '选择端点',
        runTest: '运行测试',
        loadingEndpoints: '加载中...',
        noEnabledEndpoints: '无启用的端点',
        pleaseSelectEndpoint: '请选择一个端点',
        testCompletedSuccessfully: '测试成功完成',
        testFailed: '测试失败',
        latency: '延迟',
        response: '响应',
        error: '错误',
        noResponse: '无响应',
        unknownError: '未知错误',
        failedToLoadEndpoints: '加载端点失败'
    },
    notifications: {
        endpointsReordered: '端点重新排序成功',
        endpointCreated: '端点创建成功',
        endpointUpdated: '端点更新成功',
        endpointDeleted: '端点删除成功',
        endpointCloned: '端点克隆成功',
        endpointSwitched: '已切换到端点：',
        endpointEnabled: '端点已启用',
        endpointDisabled: '端点已禁用',
        testSuccessful: '测试成功！延迟：',
        testFailed: '测试失败：',
        modelSelected: '已选择模型：',
        importDone: '导入完成：新增 {created}，更新 {updated}，跳过 {skipped}，失败 {failed}',
        credentialEnabled: '凭证已启用',
        credentialDisabled: '凭证已禁用',
        credentialActivated: '凭证已激活',
        tokenUpdated: '凭证令牌已更新',
        credentialDeleted: '凭证已删除',
        urlCopied: 'URL 已复制',
        realtimeConnected: '实时连接正常',
        realtimeDisconnected: '实时连接已中断，正在自动重连',
        realtimeRestored: '实时连接已恢复'
    },
    transformers: {
        claude: 'Claude',
        openai: 'OpenAI',
        openai2: 'OpenAI 响应',
        gemini: 'Gemini'
    },
    authModes: {
        api_key: 'API 密钥',
        token_pool: '令牌池',
        codex_token_pool: 'Codex 令牌池'
    }
};
